...
class InfoBox(ctk.CTkFrame):
    ...
    def __init__(self, parent, title, title_anchor="center", info="", info_justify="center", info_fill="none", info_placeholder=None, info_state="readonly", info_width=100, info_var=None, title_font=("Arial", 14, "bold")):
        ...
        self.info_Entry = ctk.CTkEntry(
            self, justify=info_justify, textvariable=info_var, placeholder_text=info_placeholder, width=info_width)
...
class QAForm(ctk.CTkFrame):
    def __init__(self, parent, qa_dict, header_name, padx_details=0):
        ...
        self.header = ctk.CTkLabel(
            self, text=header_name, font=("Arial", 14, "bold"))
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.DestFolder = InfoBox(self.Window1, title="Destination Folder:",
                                  title_anchor="nw", info=Ram_Dump.output, info_justify="left", info_fill="x")
        self.DestFolder.grid(row=0, column=0, columnspan=4, sticky="ew")

        self.file_name = InfoBox(self.Window1, title="Specify File Name:", info_justify="left",
                                 info_state="normal", info_fill="x", info_placeholder="memdump_YYYY-MM-DD_HH-MM-SS")
        self.file_name.grid(row=1, column=0, columnspan=3,
        ...
        self.status_frame = ctk.CTkFrame(
            self.Window1, border_width=2, border_color="lightblue")
        self.status_frame.grid(
            row=2, column=0, columnspan=4, pady=(0, 10), sticky="ew")
        ...
        self.status_label = ctk.CTkLabel(
            self.status_frame, text="Status:", font=("Arial", 14, "bold"))
        self.status_label.grid(row=0, column=0, padx=(
            10, 0), pady=(10, 0), sticky="nw")
        ...
        self.progress_bar = ctk.CTkProgressBar(
            self.status_frame, orientation="horizontal", mode="determinate", height=20)
        self.progress_bar.grid(row=1, column=0, columnspan=2,
                               padx=10, pady=(5, 20), sticky="ew")
        self.progress_bar.set(0)

        self.starttime = InfoBox(
            self.Window1, title="Start Time:", info="HH:MM:SS")
        self.starttime.grid(row=3, column=0)

        self.endtime = InfoBox(
            self.Window1, title="End Time:", info="HH:MM:SS")
        self.endtime.grid(row=3, column=1)

        self.elapsed_time = ctk.StringVar()
        self.elapsed_time.set("00:00:00")
        self.elapsedtime = InfoBox(
            self.Window1, title="Elapsed Time:", info_var=self.elapsed_time)
        self.elapsedtime.grid(row=3, column=2)

        self.osdect = InfoBox(self.Window1, title="Detected OS:",
                              info=Ram_Dump.os_name, info_width=180)
        ...
        self.QAForm_Case = QAForm(
            self.Window2, Ram_Dump.case_details, header_name="Case Details:")
        self.QAForm_Case.pack(anchor="nw")

        self.QAForm_Examiner = QAForm(
            self.Window2, Ram_Dump.examiner_details, header_name="Examiner Details:", padx_details=10)
        ...
        self.closeButton = ctk.CTkButton(
            self.button_frame, text="Close", command=self.close_clicked)
        self.closeButton.grid(padx="10", row=0, column=2)

        # Next button
        self.nextButton = ctk.CTkButton(
            self.button_frame, text="Next", state="disabled", command=self.next_clicked)
        self.nextButton.grid(row=0, column=1)

        # Capture button
        self.captureButton = ctk.CTkButton(
            self.button_frame, text="Capture!", command=self.capture_clicked)
        self.captureButton.grid(padx="10", row=0, column=0)

        # Cancel button
        self.cancelButton = ctk.CTkButton(
            self.button_frame, text="Cancel", command=self.cancel_clicked)

        # Finish button
        self.finishButton = ctk.CTkButton(
            self.button_frame, text="Finish!", command=self.finish_clicked)

    def start_timer(self):
        self.start_time = Ram_Dump.time.time()
        self.starttime.insert_text(
